...
if(isset($_REQUEST['reset'])){
  ...
  if($sql > 0){
$name = $sss['name'];
$subject = "Reset Password - MLM project";
$txt = "Hi,".$sss['name']."<br><br> You have requested to change your  password. Reset link given below click on link and reset your password <br><br><br>  https://https://mlm-project.ashifraza.in/reset-password.php?reset=" . base64_encode($sss['password']). "&verify=".base64_encode($sss['user_id'])."<br><br><br> Thanks & Regards, <br>MLM Project";

                                                
if(sendMail($email,$name,$subject,$txt)) {
...
if (isset($_REQUEST['withdrawl_request'])) {
  ...
  if ($amt>="500") {
    ...
    mysqli_query($conn,"INSERT INTO `income_history`(`user_id`, `amt`, `desp`, `datee`, `dtime`, `cr_dr`) VALUES ('$user_id','$amt','withdraw', '$date','$time', '1')");
...
if (isset($_REQUEST['pin'])) {
    ...
    if (isset($_REQUEST['user_registration'])) {
        $username = $_REQUEST['sponsor_id'];
        $pos = mysqli_real_escape_string($conn, $_REQUEST['position']);
        $sql = mysqli_query($conn, "SELECT * FROM users WHERE user_id='$username'");
        ...
        if ($poa == "left") {
            if ($row['left_side'] > 0) {
                while ($row['left_side'] > 0) {
                    $username = $row['left_side'];
                    $sql = mysqli_query($conn, "SELECT * FROM users WHERE user_id='$username'");
                    $row = mysqli_fetch_array($sql);
            ...
            if ($row['right_side'] > 0) {
                while ($row['right_side'] > 0) {
                    $username = $row['right_side'];
                    $sql = mysqli_query($conn, "SELECT * FROM users WHERE user_id='$username'");
                    $row = mysqli_fetch_array($sql);
...
function level_distribution($s_id, $prc){
  ...
  while ($a < 5 && $s_id!=0) {
    ...
    mysqli_query($conn,"INSERT INTO `income_history`(`user_id`, `amt`, `desp`, `datee`, `dtime`, `cr_dr`) VALUES ('$s_id','$income[$a]','Affiliate Commission', '$dateaa','$dtime', '0')");
...
function level_distributionl($s_id, $prc){
  ...
  while ($a < 10 && $s_id!=0) {
    mysqli_query($conn,"UPDATE `wallet` SET `amount`=`amount`+$income[$a] WHERE `user_id`='$s_id'");
    mysqli_query($conn, "INSERT INTO wallet (user_id, amount, date, status) VALUES ('$s_id', '$income[$a]', '$dateaa', '0')");
    mysqli_query($conn,"INSERT INTO `income_history`(`user_id`, `amt`, `desp`, `datee`,`dtime`, `cr_dr`) VALUES ('$s_id','$income[$a]','Level Income', '$dateaa','$dtime', '0')");
    $next_id=find_sponsor_id($s_id);
    $s_id=$next_id;
    $a++;
  }
...
function mailing($email,$name,$userid){
  ...
  $full_name = "MLM Project";
  $subject = "Verify Email - MLM Project";
  $txt = "Hi,".$name."<br><br> Please Verify Your Email. Email Verification Link Is Given Below. Simply Click  On This Link And Verify Your Email. <br><br><br> https://mlm-project.ashifraza.in/verify_email.php?verify_email=" . base64_encode($email). "&verify=".base64_encode($userid)."<br><br>Thanks & Regards, <br> MLM Project";
  sendMail($email,$name,$subject,$txt);
