                                    ...
                                    <div class="table-responsive">
                                        <table class="table table-striped table-bordered first">
                                            ...
                                            <tbody>
                                                <?php
                                                  $a=0;
                                                 $income_h_query = mysqli_query($conn, "SELECT users.user_id, users.password, users.name, users.status, SUM(wallet.amount) AS total_amount FROM users LEFT JOIN wallet ON users.user_id = wallet.user_id GROUP BY users.user_id");
                                                  while ($data=mysqli_fetch_array($income_h_query)) {
                                                    $id=$data['user_id'];
                                                    ?>
                                                      <tr>
                                                        <td><?php echo ++$a ?></td>
                                                        <td><a href="profile.php?myid=<?php echo $data['user_id'] ?>"><?php echo $data['user_id']; ?></a></td>
                                                        <td><?php echo $data['name'] ?></td>
                                                        <td><?php echo ($data['status'])?"Blocked":"unblock" ?></td>
                                                        <td><?php if($data['total_amount'] > 0){echo $data['total_amount'];}else{echo 0;} ?></td>
                                                        <td>
                                                          <form method="POST" action="../element/redirect.php">
                                                              ...
                                                              <button type="submit" name="login_by_ad" class="btn btn-primary btn-sm btn-block">Login</button>
                                                          </form>
                                                        </td>
                                                      </tr>
                                                    <?php
